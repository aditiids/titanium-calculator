...
// in simulator we redefine to format for HelloWorldApp Developer console
...
#define DEFINE_EXCEPTIONS                                                                                      \
  ...
  {                                                                                                            \
    NSString *exceptionName = [@"org.helloworldapp." stringByAppendingString:NSStringFromClass([self class])];  \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }                                                                                                            \
                                                                                                               \
  +(void)throwException : (NSString *)reason subreason : (NSString *)subreason location : (NSString *)location \
  {                                                                                                            \
    NSString *exceptionName = @"org.helloworldapp";                                                             \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }
...
enum {
  UIHelloWorldAppNativeItemNone = -1,
  UIHelloWorldAppNativeItemSpinner = -2,
  UIHelloWorldAppNativeItemProgressBar = -3,

  UIHelloWorldAppNativeItemSlider = -4,
  UIHelloWorldAppNativeItemSwitch = -5,
  UIHelloWorldAppNativeItemMultiButton = -6,
  UIHelloWorldAppNativeItemSegmented = -7,

  UIHelloWorldAppNativeItemTextView = -8,
  UIHelloWorldAppNativeItemTextField = -9,
  UIHelloWorldAppNativeItemSearchBar = -10,

  UIHelloWorldAppNativeItemPicker = -11,
  UIHelloWorldAppNativeItemDatePicker = -12,

  UIHelloWorldAppNativeItemInfoLight = -13,
  UIHelloWorldAppNativeItemInfoDark = -14,

  UIHelloWorldAppNativeItemDisclosure = -15,

  UIHelloWorldAppNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all HelloWorldApp instances of
