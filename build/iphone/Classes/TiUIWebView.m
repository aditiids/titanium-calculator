...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScriptHelloWorldAppInjectionForAppEvent]];
...
- (WKUserScript *)userScriptHelloWorldAppInjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_helloworldapp = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _helloworldapp, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScriptHelloWorldAppJSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return url to HelloWorldApp in order to handle OAuth and more
